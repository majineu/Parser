...
//------------------------------------------------------------------------------------------
bool CEvaler::
Eval(vector<CDepTree*>  &rGolds,  vector<CDepTree*>  &rGuesses,  double * pPerformance)
{
	static const wchar_t * puncts[] = {L".", L":" , L"$",L"-LRB-", L"-RRB-",L",", L"``",  L"#" ,L"''" };
	
	size_t goldTreeNum = rGolds.size();
	size_t resTreeNum  = rGuesses.size();
	...
	if (goldTreeNum != resTreeNum)
	{
		fprintf(stderr, "Error: number of goldern tree %lu, number of result tree %lu\n", 
						goldTreeNum, resTreeNum);
		return false;
	}
	
	static std::set<wstring> punctSet;
	if (punctSet.size() == 0)
		punctSet.insert(puncts,   puncts + sizeof(puncts)/sizeof(wchar_t *));
	
	vector<CDepTree*> goldTreeNodes, resTreeNodes;
	for (size_t tid = 0; tid < goldTreeNum; ++ tid)
	{
		goldTreeNodes.clear();
		resTreeNodes.clear();
		rGolds[tid]->CollectTreeNodes(goldTreeNodes);
		rGuesses[tid]->CollectTreeNodes(resTreeNodes);
		if (goldTreeNodes.size() != resTreeNodes.size())
		{
			fprintf(stderr, "ERROR: node number does match\n");
			rGolds[tid]->PrintTree(stderr, &CIDMap::GetDepLabelVec());
			rGuesses[tid]->PrintTree(stderr, &CIDMap::GetDepLabelVec());
			return false;
		}

		
		size_t nodeNum = goldTreeNodes.size();
		for (size_t i = 0; i < nodeNum; ++i)
		{
			_SENTENCE *pSen = goldTreeNodes[0]->GetSen();
			wstring tag(pSen->Tag(goldTreeNodes[i]->Index()));//GetAutom()->second;
			if (tag == L"PU" || tag == L"pu" || punctSet.find(tag) != punctSet.end())
				continue;
				
			CDepTree * goldNodeParent = goldTreeNodes[i]->GetPi();
			CDepTree * resNodeParent = resTreeNodes[i]->GetPi();
			
			/* for root node */
			if (goldNodeParent == NULL || resNodeParent == NULL)
			{
				if (goldNodeParent == resNodeParent)
				{
					++rootCorr;
					++corrNum;
					++corrLasNum;
			...
			else if (goldTreeNodes[i]->GetPi()->Index()== 
							 resTreeNodes[i]->GetPi()->Index())
			{
				++corrNum;
				if (goldTreeNodes[i]->GetDepId() == resTreeNodes[i]->GetDepId())
					++corrLasNum;
			}
			++totalNum;
		}
	...
	fprintf(stderr,"  %-4.2f%%  %-4.2f%%  %-4.2f%%  ", 100.0*corrNum/totalNum,  100.0*corrLasNum/totalNum,
				100.0*rootCorr/resTreeNum);
	if(pPerformance != NULL)
		*pPerformance = (double)corrNum/totalNum;
	return true;
}
...
CSRParser::
CSRParser(size_t bs, bool insertMode,
					CFeatureCollector *pCF):
...
Training(_SENTENCE *pSen, CDepTree *pRef)
{
	...
	{	
		...
		{	
			// the correct analysis does not in the beam 
...
Parsing(_SENTENCE *pSen)
{
	...
	for (int step = 0; step < stepNum; ++ step)
		if (updateBeam(false, m_verbose) == false)
			throw ("Error: update beam failed\n");
...
{	
	...
	for (size_t i = 0; i < m_Beam.size(); ++i)
	{
		...
		/* 4. create new state and update heap */
		for (size_t i = 0; i < CIDMap::GetOutcomeNum(); ++i)
		{
			...
			if ((action == CIDMap::SHIFT && index >= senLen) || 
					(action != CIDMap::SHIFT && (pCurState->StackEmpty() || 
					 pCurState->GetStackLen() < 2)))
				continue;
...
{
	bool survive = false;
	static vector<CState*> ::iterator it, end;

	// check goldEvent survive 
	it = m_Beam.begin(), end = m_Beam.end();
	if (lastStep)
		it += m_Beam.size() - 1;
